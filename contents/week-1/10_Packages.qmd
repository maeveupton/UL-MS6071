...
<br> Much of R functionality is provided via packages. Packages are a collection of functions, data and documentation that are created to perform a particular task(s). Some packages are part of the base R installation (e.g., `base`, `mgcv`, `nlme`, `stats`, ...). Others can be downloaded and installed from [CRAN](https://cran.r-project.org/web/packages/available_packages_by_name.html). Packages can be created and shared by anyone in the R community.
